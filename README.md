...
```text
.
├── index.php                         # Main application and UI
├── Readme.md                         # This documentation
├── .replit                           # Optional Replit workflow configuration
├── .guardian-server-router.php       # PHP built-in-server recovery router
├── .guardian-restore.php             # Generated recovery endpoint when enabled
├── .fg_*/                            # Generated Guardian metadata and backups
├── attached_assets/
│   └── fonts/tmt.ttf                 # Cached terminal font
└── screenshots/
    └── terminal-manager-check.jpg    # UI reference screenshot
```

The source intentionally keeps the main UI and server actions together in `index.php`, which makes deployment simple but means that every update should be reviewed as a privileged server-side change.
...
> **Replace `https://t.me/your_channel` with the official Telegram URL before publishing.** No Telegram channel URL is currently present in the source files of this snapshot, so a real channel link cannot be inferred safely.
